#define S_FUNCTION_NAME  nrf5_flash
...
static void mdlInitializeSizes(SimStruct *S) {
    ...
    for(k=0; k<input_count; k++) {		
        ...
        if(k<input_width_count) {
            ...
            //ssSetInputPortWidth(S, k, (wiC:\Users\Navy\waijung15_04a_original\targets\nrf5_target\nrf5\demo\uart_demodth>0)?width:1);
			if (width > 0) {
...
#define FLASH_DATA_BUFFER_SIZE (256*1024) // 128k
...
static void mdlOutputs(SimStruct *S, int_T tid) {
	...
	if ((Address >= (0x40000 | FLASH_DATA_BUFFER_SIZE)) || (Address < 0x1D000)) {
		ssSetErrorStatus(S, "\nInvalid memory Offset/ Address.\n");
		return;
	}
	// BufferPointer
	BufferPointer = Address - 0x1D000;
 ...
 * (e.g. must have TLC file nrf5_flash.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function nrf5_flash.c"
#endif
