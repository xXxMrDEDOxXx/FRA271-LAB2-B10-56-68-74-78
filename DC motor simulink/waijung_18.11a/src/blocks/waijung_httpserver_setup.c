#define S_FUNCTION_NAME  waijung_httpserver_setup
...
static void mdlInitializeSizes(SimStruct *S) {
    ...
    for(k=0; k<output_count; k++) {
        if(k<output_width_count)
            ssSetOutputPortWidth(S, k, (int)(((double*)mxGetPr(ssGetSFcnParam(S, ARGC_OUTPUT_PORTWIDTH)))[k]));
        else
    ...
    for(k=0; k<input_count; k++) {
        ...
        if(k<input_width_count)
            ssSetInputPortWidth(S, k, (int)(((double*)mxGetPr(ssGetSFcnParam(S, ARGC_INPUT_PORTWIDTH)))[k]));
        else
 ...
 * (e.g. must have TLC file waijung_httpserver_setup.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function waijung_httpserver_setup.c"
#endif
