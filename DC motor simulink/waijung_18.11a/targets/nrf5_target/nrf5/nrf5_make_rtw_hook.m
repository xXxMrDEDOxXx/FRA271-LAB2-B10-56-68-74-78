function nrf5_make_rtw_hook(hookMethod,modelName,rtwroot,templateMakefile,buildOpts,buildArgs)
...
switch hookMethod
    ...
    case 'entry'
        ...
        eval([targetname '_entry_hook(modelName)']);
    case 'before_tlc'
        % Called just prior to invoking TLC Compiler (actual code generation.)
        % Valid arguments at this stage are hookMethod, modelName, and
        % buildArgs
        eval([targetname '_before_tlc_hook(modelName)']);
    case 'after_tlc'
        % Called just after to invoking TLC Compiler (actual code generation.)
        % Valid arguments at this stage are hookMethod, modelName, and
        % buildArgs
        eval([targetname '_after_tlc_hook(modelName)']);
    case 'before_make'
        % Called after code generation is complete, and just prior to kicking
        % off make process (assuming code generation only is not selected.)  All
        % arguments are valid at this stage.
        eval([targetname '_before_make_hook(modelName)']);
    case 'after_make'
        % Called after make process is complete. All arguments are valid at
        % this stage.
        eval([targetname '_after_make_hook(modelName)']);
    case 'exit'
        % Called at the end of the RTW build process.  All arguments are valid
        % at this stage.
        eval([targetname '_exit_hook(modelName)']);
