...
/* Usage:
 * 1. Check connection:
 *    => [sta, msg] = amg_usb_connect_nrf51('connect')
 *    Where, sta = 1 Success, 0 - Error.
 *
 * 2. Write program:
 *    => [sta, msg] = amg_usb_connect_nrf51('writeflash', <membase>, <memsize>, <filename>);
 *    =>Example, [sta, msg] = amg_usb_connect_nrf51('writeflash', hex2dec('20000'), hex2dec('1C800'), 'digital_io_demo.bin');
 *    =>Example, [sta, msg] = amg_usb_connect_nrf51('writeflash', hex2dec('3E000'), hex2dec('1C00'), 'secondaryboot_iap.bin');
 *    Where, sta = 1 Success, 0 - Error.
 *
 * 3. Full Erase:
 *    => [sta, msg] = amg_usb_connect_nrf51('fullerase');
 *    Where, sta = 1 Success, 0 - Error.
 *
 * 4. Run:
 *    => [sta, msg] = amg_usb_connect_nrf51('run');
 *    Where, sta = 1 Success, 0 - Error.
 * 5. Use primary bootloader
 *    => [sta, msg] = amg_usb_connect_nrf51('primaryboot');
 *    Where, sta = 1 Success, 0 - Error.
 */
	 ...
	 * === Command: readtofile === */
		...
		if ((nrhs == 4)
			...
			if (fp) {				
				...
				ftHandle = IAP_Comm_Open();
				if (ftHandle != 0) {
					if ((mem_size > 0) && (mem_size <= 0x40000)) {
						/* Connect */
						output_sta = (int)IAP_Init(ftHandle, &IapCmdSupport, &OptionsByte, &Pid, &Bootloader_version);
						if (output_sta != 0) {
							/* Read mem */
							...
							if (output_sta != 0) {
								...
								if ((output_sta != 0) && (Bootloader_version != 0x10)) {
									...
									if ((file_size & 0x3FF) != 0)
										page_count = ((file_size | 0x3FF) + 1) / 1024;
									else {
										page_count = file_size / 1024;
									}
									mexPrintf("Erasing...\n");
									if (!IAP_EraseFlash_Page(ftHandle, (unsigned char)(mem_address/1024), (unsigned char)page_count)) {
										output_sta = 0;
									}
									if (output_sta)
										mexPrintf("\tdone\n");
...
/*
								if (output_sta != 0) {
									...
									while (flash_size > 0) {
										if (!IAP_Erase_Ext_Flash(ftHandle, page_idx)) {
											mexPrintf("\nError: failed to Erase flash. Please try again.\n");
											output_sta = 0;
										}
										page_idx ++;
	...
	/* === Command: fullerase === */
		...
		if (ftHandle != 0) {
			...
			if (output_sta) {
				...
				// CODE_REGION_1_START				
				if (IAP_EraseFlash_Page(ftHandle, (unsigned char)(CODE_REGION_1_START/1024), 114)) {
					mexPrintf("Erase success.\n");
				}
				else {
					output_sta = 0;
