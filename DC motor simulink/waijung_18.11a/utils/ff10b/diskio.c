...
__try_read:
    ...
    if((unsigned int)buff & 3) {
		i = 0;
		p = buff;
		while ((Status == SD_OK) && (i < count)) {
			Status = SD_ReadMultiBlocks((uint8_t*)tmp, (sector + (DWORD)i)*SECTOR_SIZE, SECTOR_SIZE, 1);
			memcpy(p,tmp,SECTOR_SIZE);
			p += SECTOR_SIZE;
			i++;
		}
	}
...
DRESULT SDCARD_disk_write(const BYTE *buff, DWORD sector, UINT count)
{
	...
	if (((unsigned int)buff & 3) == 0) {
		...
		i = 0;
		p = buff;
		while ((Status == SD_OK) && (trans_state == SD_TRANSFER_OK) && (i < count)) {
			memcpy(tmp, p, SECTOR_SIZE);
			Status = SD_WriteMultiBlocks((uint8_t*)tmp, (sector+(DWORD)i)*SECTOR_SIZE, SECTOR_SIZE, 1);
			p += SECTOR_SIZE;
			i++;
