#define S_FUNCTION_NAME  waijung_stringbuffer_compare
...
static void mdlRTW(SimStruct *S) {
    ...
    method = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_METHOD));
    blockid = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_BLOCKID));
    
    NOutputPara = 4; /* Number of parameters to output to model.rtw */
    if (!ssWriteRTWParamSettings(S, NOutputPara,
            SSWRITE_VALUE_QSTR, "configuration", configuration,
            SSWRITE_VALUE_QSTR, "method", method,
...
#error "Attempted use non-inlined S-function waijung_stringbuffer_compare.c"
#endif
