...
/* #########################################################################
   ...
   #########################################################################
*/
SD_Error CmdResp1Error_nonblock(uint8_t cmd, uint8_t init, uint8_t *state)
{
  static uint8_t cmd_state = 0;
  static SD_Error errorstatus = SD_OK;
...
SD_Error SD_WriteMultiBlocks_nonblock(uint8_t *writebuff, uint64_t WriteAddr64, uint16_t BlockSize, uint16_t NumberOfBlocks, uint8_t init, uint8_t *state)
{
  static SDIO_CmdInitTypeDef SDIO_CmdInitStructure_nonblock;
  ...
  {
    BlockSize = 512;
	WriteAddr = (uint32_t)((uint64_t)WriteAddr64/512U);
  }
  else {
	  WriteAddr = (uint32_t)WriteAddr64;
  }
  ...
  switch(write_state) {
	...
	case 1:
		/* Set Block Size for Card */ 
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = (uint32_t) BlockSize;
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_SET_BLOCKLEN;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SET_BLOCKLEN, 1, &sub_state);
		write_state++;
		break; /* Can removed */

	case 2: /* wait from state 1 */
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SET_BLOCKLEN, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			if (SD_OK != errorstatus) { /* Stop due to error */
				write_state = 0;
				break;
			}
			else {
	...
	case 3:
		/*!< To improve performance */
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = (uint32_t) (RCA << 16);
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_APP_CMD;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_APP_CMD, 1, &sub_state);
		write_state++;
	case 4: /* Continue from state 3 */
		errorstatus = CmdResp1Error_nonblock(SD_CMD_APP_CMD, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			if (SD_OK != errorstatus) { /* Stop due to error */
				write_state = 0;
				break;
	...
	case 5:
		/*!< To improve performance */
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = (uint32_t)NumberOfBlocks;
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_SET_BLOCK_COUNT;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SET_BLOCK_COUNT, 1, &sub_state);
		write_state++;
	case 6: /* Continue from state 5 */
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SET_BLOCK_COUNT, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			if (SD_OK != errorstatus) { /* Stop due to error */
				write_state = 0;
				break;
	...
	case 7:
		/*!< Send CMD25 WRITE_MULT_BLOCK with argument data address */
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = (uint32_t)WriteAddr;
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_WRITE_MULT_BLOCK;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_WRITE_MULT_BLOCK, 1, &sub_state);
		write_state++;
	case 8:
		errorstatus = CmdResp1Error_nonblock(SD_CMD_WRITE_MULT_BLOCK, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			if (SD_OK != errorstatus) { /* Stop due to error */
				write_state = 0;
				break;
...
SD_Error SD_StopTransfer_nonblock(uint8_t init, uint8_t *state)
{
  static SDIO_CmdInitTypeDef SDIO_CmdInitStructure_nonblock;
  ...
  switch(stop_state) {
	...
	case 1:
		/*!< Send CMD12 STOP_TRANSMISSION  */
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = 0x0;
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_STOP_TRANSMISSION;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_STOP_TRANSMISSION, 1, &sub_state);
		stop_state++;
		break; /* Can removed */
	case 2:
		errorstatus = CmdResp1Error_nonblock(SD_CMD_STOP_TRANSMISSION, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			stop_state = 0;
		}
		break;
...
SYS_TIMER_STRUCT amg_sdio_timer;
SD_Error SD_WaitWriteOperation_nonblock(uint8_t init, uint8_t *state)
{
  static uint8_t wait_timeout1 = 0;
  static uint8_t wait_timeout2 = 0;
  static uint8_t wait_state = 0;
  static SD_Error errorstatus = SD_OK;
  uint8_t sub_state = 0;
		
  if(init) {
	errorstatus = SD_OK;
	SysTimer_Start(&amg_sdio_timer, 1000); /* 1S timeout */
    wait_state = 1;
	wait_timeout1 = 0;
	wait_timeout2 = 0;
  }
  ...
  switch(wait_state) {
	case 0: /* Idle */
		break;
	case 1:
		wait_timeout1 = SysTimer_IsTimeout(&amg_sdio_timer);
		if((DMAEndOfTransfer == 0x00) && (TransferEnd == 0) && (TransferError == SD_OK) && (wait_timeout1 == 0)) {
			break;
		}
		else {
			DMAEndOfTransfer = 0x00;
	...
	case 2:
		wait_timeout2 = SysTimer_IsTimeout(&amg_sdio_timer);
		if((SDIO->STA & SDIO_FLAG_TXACT) && (wait_timeout2 == 0)) {
			break;
		}
		else {
			wait_state++;
	...
	case 3:
		if (StopCondition == 1) {
			errorstatus = SD_StopTransfer_nonblock(1, &sub_state);
		}
		wait_state++;
		break; /* Can removed */
	case 4:
		if (StopCondition == 1) {
			errorstatus = SD_StopTransfer_nonblock(0, &sub_state);
			if(sub_state == 0) {
				wait_state++;
				StopCondition = 0;
			}
	...
	case 5:
		...
		if ((wait_timeout1 != 0) && (wait_timeout2 != 0) && (errorstatus == SD_OK))
		{
			errorstatus = SD_DATA_TIMEOUT;
		}
		/*!< Clear all the static flags */
...
SD_Error SD_SendStatus_nonblock(uint32_t *pcardstatus, uint8_t init, uint8_t *state)
{
  static SDIO_CmdInitTypeDef SDIO_CmdInitStructure_nonblock;
  ...
  switch(send_state) {
	...
	case 1:
		SDIO_CmdInitStructure_nonblock.SDIO_Argument = (uint32_t) RCA << 16;
		SDIO_CmdInitStructure_nonblock.SDIO_CmdIndex = SD_CMD_SEND_STATUS;
		SDIO_CmdInitStructure_nonblock.SDIO_Response = SDIO_Response_Short;
		SDIO_CmdInitStructure_nonblock.SDIO_Wait = SDIO_Wait_No;
		SDIO_CmdInitStructure_nonblock.SDIO_CPSM = SDIO_CPSM_Enable;
		SDIO_SendCommand(&SDIO_CmdInitStructure_nonblock);
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SEND_STATUS, 1, &sub_state);
		send_state++;
		break; /* Can removed */
	case 2:
		errorstatus = CmdResp1Error_nonblock(SD_CMD_SEND_STATUS, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			send_state =0;
			if(errorstatus == SD_OK) {
				*pcardstatus = SDIO_GetResponse(SDIO_RESP1);
...
SDCardState SD_GetState_nonblock(uint8_t init, uint8_t *state)
{
  ...
  switch(get_state) {
	case 0: /* Idle */
		break;
	case 1:
		errorstatus = SD_SendStatus_nonblock(&resp1, 1, &sub_state);
		get_state++;
		break; /* Can removed */
	case 2:
		errorstatus = SD_SendStatus_nonblock(&resp1, 0, &sub_state);
		if(sub_state == 0) { /* Done */
			get_state = 0;
		}
  }
...
SDTransferState SD_GetStatus_nonblock(uint8_t init, uint8_t *state)
{
  static uint8_t status_state = 0;
  static SDCardState cardstate =  SD_CARD_TRANSFER;
  uint8_t sub_state;
  ...
  switch(status_state) {
	  ...
	  case 1:
		  cardstate = SD_GetState_nonblock(1, &sub_state);
		  status_state++;
		  break; /* Can removed */
	  case 2:
		  cardstate = SD_GetState_nonblock(0, &sub_state);
		  if(sub_state == 0) {/* Done */
			status_state = 0;
		  }
