#define S_FUNCTION_NAME  waijung_string_processing
...
enum {
	...
	ARGC_STRING_FUNCTION,
	ARGC_STRING_FORMAT,
	ARGC_BEGINDELIM_FORMAT,
	ARGC_ENDDELIM_FORMAT,
	
	ARGC_BUFFER_SIZE,
	ARGC_ENABLE_STATUS,
	ARGC_DATATYPE_STRING,
...
static void mdlRTW(SimStruct *S) {
	...
	stringfunction = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_FUNCTION));
	stringformat = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_FORMAT));
	datatypestring = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_DATATYPE_STRING));
	
	begin_delim = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_BEGINDELIM_FORMAT));
	end_delim = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_ENDDELIM_FORMAT));
	
	if (!ssWriteRTWParamSettings(S, NOutputPara,
			SSWRITE_VALUE_QSTR, "conf", conf,
			SSWRITE_VALUE_QSTR, "stringfunction", stringfunction,
			SSWRITE_VALUE_QSTR, "stringformat", stringformat,
			SSWRITE_VALUE_QSTR, "begin_delim", begin_delim,
			SSWRITE_VALUE_QSTR, "end_delim", end_delim,
			SSWRITE_VALUE_NUM, "buffersize", mxGetScalar(ssGetSFcnParam(S, ARGC_BUFFER_SIZE)),
			SSWRITE_VALUE_NUM, "enablestatus", mxGetScalar(ssGetSFcnParam(S, ARGC_ENABLE_STATUS)),
    ...
    if (!ssWriteRTWStrVectParam(S, "datatypestring", datatypestring, get_list_count(datatypestring))){
        return;
    }    
        
    mxFree(conf);
	mxFree(stringfunction);
	mxFree(stringformat);
	mxFree(datatypestring);
 ...
 * (e.g. must have TLC file waijung_string_processing.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function waijung_string_processing.c"
#endif
