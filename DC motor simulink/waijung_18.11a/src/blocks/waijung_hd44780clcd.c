#define S_FUNCTION_NAME  waijung_hd44780clcd
#define S_FUNCTION_LEVEL 2
#include "simstruc.h"
#define NPAR 6 /* Total number of block parameters */
...
#define ADVANCEDOPTIONS(S) mxGetScalar(ssGetSFcnParam(S, 2)) /* Advanced options */
#define SAMPLETIME(S) mxGetScalar(ssGetSFcnParam(S, 3)) /* Sample time (sec) */
#define SAMPLETIMESTR(S) (char*)mxArrayToString(ssGetSFcnParam(S, 4)) /* Compiled sample time (sec) in string */
#define BLOCKID(S) (char*)mxArrayToString(ssGetSFcnParam(S, 5)) /* BlockID */
...
static void mdlRTW(SimStruct *S) {
	...
	if (!ssWriteRTWParamSettings(S, NOutputPara,
			...
			SSWRITE_VALUE_NUM, "advancedoptions", ADVANCEDOPTIONS(S),
 ...
 * (e.g. must have TLC file waijung_hd44780clcd.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function waijung_hd44780clcd.c"
#endif
