...
{
  static int non_blocking_substate = 0; /* Process sub-state for nonblocking */
  ...
  if(*nonblocking_state == 0) {
	  ...
	  _eth_delay_(PHY_RESET_DELAY);
  	  ...
  	  /* --- Next --- */
	  *nonblocking_state = 1;
	  non_blocking_substate = 0;
	  //timeout = 0;

  } /* *nonblocking_state == 0 */
 
  if(*nonblocking_state == 1) {

	  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
	  {  
			switch(non_blocking_substate) {
			/* We wait for linked status... */
			case 0:
				//do
				//{
...
{ 
  ...
  */
  static FrameTypeDef frame = {0,0,0};  /* Krisada: use static */
