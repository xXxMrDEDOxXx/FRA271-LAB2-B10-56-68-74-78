...
int fs_open_custom(struct fs_file *file, const char *name)
{
	...
	if(pszExt && ((strcmp(pszExt, "txt") == 0) || (strncmp((char*)name, "/html_", 6) == 0) || (strncmp((char*)name, "html_",5) == 0)) {
		/* Remove slash */
		if(*(varname=(char*)name) == '/')
			varname++;

		if (strncmp((char*)varname, "html_",5) == 0) {
			varname+=5;
		}
		
		/* If variable name is valid */
		if((var_index = vars_get_index(varname, pszExt-varname-1)) >= 0) {				
			/* Get var to buffer */
	...
	/* Variables request as list */
		...
		while (((pos = strstr(nam, ",")) != (void*)0) || ((pos = strstr(nam, ".")) != (void*)0)) {
			...
			if((var_index = vars_get_index(tmp_filename, strlen(tmp_filename))) >= 0) {
				/* Get variable value */
