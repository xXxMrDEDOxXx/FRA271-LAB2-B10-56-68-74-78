...
SD_Error SD_WaitReadOperation(void)
{
  ...
  SysTimer_uS_Start(&timer, 3000000UL); /* 3 sec */
  
  while ((DMAEndOfTransfer == 0x00) && (TransferEnd == 0) && (TransferError == SD_OK) && !SysTimer_uS_IsTimeout(&timer))
  {
  }
  
  DMAEndOfTransfer = 0x00;
  
  while(((SDIO->STA & SDIO_FLAG_RXACT)) && !SysTimer_uS_IsTimeout(&timer))
  {
  }
  ...
  if (SysTimer_uS_IsTimeout(&timer) && (errorstatus == SD_OK))
  {
    errorstatus = SD_DATA_TIMEOUT;
  }
...
SD_Error SD_WaitWriteOperation(void)
{
  ...
  SysTimer_uS_Start(&timer, 3000000); /* 3 sec */
  ...
  while(((SDIO->STA & SDIO_FLAG_TXACT)) && !SysTimer_uS_IsTimeout(&timer))
  {
  }
