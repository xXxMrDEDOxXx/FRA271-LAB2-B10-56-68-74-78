#define S_FUNCTION_NAME  waijung_atcmd_gsmgprs
...
static void mdlRTW(SimStruct *S) {
	...
	string1 = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_PARAM1));
	string2 = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_PARAM2));
	string3 = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_PARAM3));
	string4 = (char*)mxArrayToString(ssGetSFcnParam(S, ARGC_STRING_PARAM4));
    
    if (!ssWriteRTWParamSettings(S, NOutputPara,
            SSWRITE_VALUE_QSTR, "conf", conf,
			SSWRITE_VALUE_QSTR, "string1", string1,
			SSWRITE_VALUE_QSTR, "string2", string2,
			SSWRITE_VALUE_QSTR, "string3", string3,
			SSWRITE_VALUE_QSTR, "string4", string4,
 ...
 * (e.g. must have TLC file waijung_atcmd_gsmgprs.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function waijung_atcmd_gsmgprs.c"
#endif
