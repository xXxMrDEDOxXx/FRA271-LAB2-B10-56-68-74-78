#define S_FUNCTION_NAME  waijung_crc
...
static void mdlInitializeSizes(SimStruct *S) {
    ...
    for(k=0; k<input_count; k++) {		
        ...
        if(k<input_width_count) {
			...
			//ssSetInputPortWidth(S, k, (width>0)?width:DYNAMICALLY_SIZED);
			if (width > 0) {
				ssSetInputPortWidth(S, k, width);
			}
			else {
				ssSetInputPortWidth(S, k, DYNAMICALLY_SIZED);
...
static void mdlSetDefaultPortDimensionInfo(SimStruct *S) {
	...
	count = (int)mxGetNumberOfElements(ssGetSFcnParam(S, ARGC_INPUT_PORTTYPE));
    for (k = 0; k < count; k++){
...
void gen_crc_table(void)
{
  ...
  register unsigned int accum;

  for (i=0; i<256; i++) {
    accum = ( (unsigned int) i << 24 );
    for ( j = 0;  j < 8;  j++ ) {
      if ( accum & 0x80000000L )
...
unsigned int crc32_update(unsigned int initial, const unsigned int *buffer, int count)
{
   unsigned int i, accum;

   accum = initial;
   for (i=0; i<count; i++) {
     accum = (accum << 8) ^ crc_table[((int) (accum >> 24) ^ (buffer[i] >> 24)) & 0xFF]; // Byte 3
	 accum = (accum << 8) ^ crc_table[((int) (accum >> 24) ^ (buffer[i] >> 16)) & 0xFF]; // Byte 2
	 accum = (accum << 8) ^ crc_table[((int) (accum >> 24) ^ (buffer[i] >> 8 )) & 0xFF]; // Byte 1
	 accum = (accum << 8) ^ crc_table[((int) (accum >> 24) ^ (buffer[i] >> 0 )) & 0xFF]; // Byte 0
   }
   return accum;
}

static void mdlOutputs(SimStruct *S, int_T tid) {
	uint32_T initial, output;
	
	// Calculate
	initial = *(const uint32_T*) ssGetInputPortSignal(S, 0);	
	output = (uint32_T)crc32_update((unsigned int)initial, \
			(const unsigned int *) ssGetInputPortSignal(S, 1), \
			ssGetInputPortWidth(S, 1));
	
	// Output
	*((uint32_T *) ssGetOutputPortSignal(S, 0)) = output;
} /* end mdlOutputs */
 ...
 * (e.g. must have TLC file waijung_crc.tlc)  *
 *=======================================*/
#ifdef    MATLAB_MEX_FILE  /* Is this file being compiled as a MEX-file?    */
#include "simulink.c"     /* MEX-file interface mechanism                  */
#else                      /* Prevent usage by RTW if TLC file is not found */
#error "Attempted use non-inlined S-function waijung_crc.c"
#endif
